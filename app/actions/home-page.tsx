...
const ORGANIZATION_ID = (import.meta as ImportMeta & { env?: Record<string, string | undefined> }).env?.VITE_ORGANIZATION_ID || ''
...
export function HomePage() {
  const cards = [
    ['integration-instructions', 'Embed cleanly', 'Web Component integration, Shadow DOM isolation, themes, and hash or memory routing.'],
    ['event', 'Run the event', 'Landing pages, folders, breadcrumbs, search, sorting, stats, and maps.'],
    ['photo-library', 'Show every memory', 'Image and video galleries, layouts, lazy loading, captions, downloads, and PhotoSwipe.'],
    ['cloud-upload', 'Collect uploads', 'QR uploads, progress, retry, cancel, HEIC conversion, crop, trim, and post-processing.'],
    ['celebration', 'Make it social', 'Face and keyword search, reactions, ratings, contests, points, and leaderboards.'],
    ['shield-lock', 'Keep people safe', 'Authentication, roles, permissions, moderation, terms, and anonymous access.'],
    ['palette', 'Fit the brand', 'Logos, covers, fonts, palettes, tokens, translations, and accessible states.'],
    ['monitor-heart', 'Operate confidently', 'Admin review, analytics, error reporting, and reliable loading and error states.'],
  ]
  return () => <Document title="Embed an Event Gallery in Remix | Qrati Connect" head={<HomeHead />}><CookieConsentBanner /><ThemeButton /><main mix={css({ minHeight: '100vh', padding: '64px 20px 24px', color: 'var(--text-primary)', background: 'linear-gradient(180deg,#fffdfb,#fff7f1)', fontFamily: 'Inter,system-ui,sans-serif' })}><header mix={css({ maxWidth: '1024px', margin: '0 auto', textAlign: 'center' })}><p mix={css({ color: '#ff4800', fontWeight: 800, letterSpacing: '.18em', textTransform: 'uppercase' })}>Embeddable Remix Gallery SDK</p><h1 mix={css({ fontSize: 'clamp(2.2rem,5vw,4rem)', letterSpacing: '-.04em' })}><a href="https://qrati.com">Qrati</a> Connect inside a Remix host site</h1><p mix={css({ maxWidth: '720px', margin: '20px auto', fontSize: '18px', lineHeight: 1.7 })}>Embed a live event photo gallery with guest uploads, lightbox, search, reactions, and contest leaderboards. Remix renders the host page; Qrati Connect renders the complete event experience.</p></header><section aria-label="Interactive Remix Event Gallery" mix={css({ maxWidth: '1024px', minHeight: '420px', margin: '48px auto', padding: '16px', border: '2px solid #ff480055', borderRadius: '24px', background: '#fff8' })}><h2>Live Event Photo Gallery Component</h2><qrati-connect organization-id={ORGANIZATION_ID} theme="light" router="hash"></qrati-connect></section><section mix={css({ maxWidth: '1024px', margin: '48px auto', padding: '28px', border: '1px solid #ff480055', borderRadius: '24px', background: '#fff8' })}><p mix={css({ color: '#ff4800', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.12em' })}>The short answer</p><h2>What does Qrati Connect add to a Remix site?</h2><p mix={css({ maxWidth: '760px', lineHeight: 1.7 })}>Qrati Connect adds a complete hosted event-media experience: guests can discover galleries, upload photos and videos, search, react, rate, and join contests while your team controls access, branding, and moderation.</p><p mix={css({ fontWeight: 700 })}>Active Qrati subscription required. Your organization ID selects the event space, settings, branding, access rules, and enabled features.</p></section><section mix={css({ maxWidth: '1024px', margin: '48px auto' })}><p mix={css({ color: '#ff4800', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.12em', textAlign: 'center' })}>Complete capability map</p><h2 mix={css({ textAlign: 'center' })}>One embed. The full event experience.</h2><div mix={css({ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '16px' })}>{cards.map(([icon, title, description]) => <article key={title} mix={css({ padding: '22px', border: '1px solid #8884', borderRadius: '18px', background: '#fff9', transition: 'transform 180ms ease, border-color 180ms ease', '&:hover': { transform: 'translateY(-3px)', borderColor: '#ff4800' } })}><div mix={css({ display: 'flex', gap: '10px', alignItems: 'center' })}><span class="iconify" data-icon={`material-symbols:${icon}`} aria-hidden="true"></span><h3 mix={css({ margin: 0, fontSize: '16px' })}>{title}</h3></div><p mix={css({ lineHeight: 1.6 })}>{description}</p></article>)}</div></section><section mix={css({ maxWidth: '760px', margin: '48px auto', padding: '32px', border: '1px solid #ff480055', borderRadius: '24px', textAlign: 'center', background: 'linear-gradient(135deg,#fff8,#fff0e9)' })}><h2>Bring every guest moment together.</h2><p>Start on Qrati, then connect the live gallery to your Remix site in minutes.</p><a href="https://qrati.com/contact-us" mix={css({ display: 'inline-flex', padding: '13px 20px', borderRadius: '999px', background: '#ff4800', color: '#fff', fontWeight: 700, textDecoration: 'none' })}>Talk to Qrati →</a></section><footer mix={css({ maxWidth: '1024px', margin: '48px auto 0', display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap', color: '#6b7280' })}><img src="https://assets.qrati.com/images/qrati-connect-logo-square.png" alt="Qrati Connect logo" width="36" height="36" /><b><span>Qrati</span> Connect</b><a href="https://qrati.com/pricing">Pricing</a><a href="https://www.npmjs.com/package/@qratilabs/qrati-connect">npm</a><span>© 2026 Qrati Labs. All rights reserved.</span></footer></main></Document>
}
function HomeHead() {
  return () => (
    <>
      <meta name="description" content="Embed Qrati Connect in Remix for live event galleries, guest uploads, search, reactions, contests, and moderation." />
      <link rel="canonical" href="https://qrati.com/connect/remix-example" />
      <meta property="og:type" content="website" />
      <meta property="og:title" content="Embed an Event Gallery in Remix | Qrati Connect" />
